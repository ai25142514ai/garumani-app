...
# 3. 表示ロジック
st.markdown('<div class="header-container"><h1 class="main-title">Garumani Analytics</h1><div class="sub-brand">Market Insight</div></div>', unsafe_allow_html=True)
...
if not df.empty:
    ...
    for _, row in df.iterrows():
        hit = row['dl'] >= 10000
        card_class = "work-card neon-hit" if hit else "work-card"
        label = '<div class="neon-label">10K+ TRENDING</div>' if hit else ""
        tags = "".join([f'<span class="tag-item">{t}</span>' for t in row['genres'].split('|')])
        st.markdown(f'<div class="{card_class}">{label}<div class="rank-text">{row['rank']:02d}</div><img src="{row['img']}" class="work-image"><div class="work-content"><div class="work-title">{row['title']}</div><div style="color:#888;font-size:0.8rem;">{row['circle']}</div><div class="tag-container">{tags}</div><div class="stats-row"><span>PRICE: ¥{row['price']:,}</span><span class="dl-text">TOTAL: {row['dl']:,} DL</span></div></div></div>', unsafe_allow_html=True)
else:
