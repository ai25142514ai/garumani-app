...
DB_NAME = "garumani_pro_v_stable_final.db"
...
def fetch_data():
    ...
    try:
        ...
        if res.status_code != 200:
            st.error(f"DLsiteへの接続に失敗しました (Error: {res.status_code})")
            return False
        soup = BeautifulSoup(res.content, "html.parser")
        items = soup.select(".n_worklist_item") or soup.select(".work_1column")
        if not items:
            st.warning("データが見つかりませんでした。サイト構造が変わった可能性があります。")
            return False
        results = []
...
# 3. 表示ロジック
st.markdown('<div class="header-box"><h1 class="main-title">Garumani Analytics</h1><div style="color:#ff4d7d;font-weight:bold;letter-spacing:2px;font-size:0.7rem;">MARKET INSIGHT</div></div>', unsafe_allow_html=True)
...
if not df.empty:
    ...
    [span_6](start_span)st.plotly_chart(fig, width="stretch") # 警告対策済[span_6](end_span)
