...
def get_session():
    ...
    session.get("https://www.dlsite.com/girls-touch/", headers=HEADERS, cookies=COOKIES, timeout=15)
    return session
...
def get_work_detail(session, url):
    try:
        ...
        img = ""
        img_el = (
            soup.select_one("#work_img_main img") or
            soup.select_one(".slider_img img") or
        ...
        price_el = (
            soup.select_one(".price_container .price") or
            soup.select_one(".work_buy_content .price")
        )
        if price_el:
        ...
        print(f"  詳細取得エラー: {e}")
        return {"circle":"","author":"","release_date":"","dl_count":0,"price":0,"tags":[],"img":""}
...
def main():
    ...
    for item in ranking_items:
        ...
        results.append({"rank":item["rank"],"title":item["title"],"url":item["url"],**detail})
        time.sleep(0.8)

    with open("ranking_data.json", "w", encoding="utf-8") as f:
        json.dump({"date":now,"date_key":date_key,"items":results}, f, ensure_ascii=False, indent=2)

    history = load_history()
    history = [h for h in history if h.get("date_key") != date_key]
    history.append({"date":now,"date_key":date_key,"items":results})
    ...
    with open("tag_ranking.json", "w", encoding="utf-8") as f:
        json.dump({"date":now,"date_key":date_key,"tags":[{"tag":t,"count":c} for t,c in top_tags]}, f, ensure_ascii=False, indent=2)
